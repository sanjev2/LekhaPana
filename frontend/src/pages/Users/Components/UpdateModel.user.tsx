...
const UpdateModel = ({ visible, setVisible, id }: any) => {
  const { isLoading, data } = useGetConsumersQuery(id);
  ...
  const onSubmitHandler = async (values: any, { setValues }: any) => {
    try {
      const { data: responseData, error }: any = await updateConsumer({ data: values, id });
      if (error) {
        ...
        {({ values, setFieldValue, handleSubmit }) => (
          <form onSubmit={handleSubmit} className="w-full">
            <div className="mb-3">
              ...
              <ErrorMessage name="name" component="p" className="text-red-500 capitalize" />
            </div>
            <div className="mb-3">
              ...
              <ErrorMessage name="email" component="p" className="text-red-500 capitalize" />
            </div>
            <div className="mb-3">
              ...
              <ErrorMessage name="mobile" component="p" className="text-red-500 capitalize" />
            </div>
            <div className="mb-3">
              ...
              />
              <ErrorMessage name="dob" component="p" className="text-red-500 capitalize" />
            </div>
            <div className="mb-3">
              <label htmlFor="address">Address <span className="text-red-500 text-sm">*</span></label>
              <Field as="textarea" rows={3} name="address" id="address" placeholder="Enter Consumer Address" className="w-full my-2 border outline-none py-3 px-4" />
              <ErrorMessage name="address" component="p" className="text-red-500 capitalize" />
            </div>
            <div className="flex justify-end">
              <Button loading={updateConsumerResponse.isLoading} className="text-white px-5 rounded-sm bg-indigo-500 py-3 text-center">
                Update Consumer
              </Button>
            </div>
          </form>
